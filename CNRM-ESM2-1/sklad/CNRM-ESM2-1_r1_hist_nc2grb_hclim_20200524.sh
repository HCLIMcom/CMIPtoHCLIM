...
#------------------------------
 ...
 path_out='/nobackup/rossby24/users/sm_grini/Data/TEMP/CNRM-ESM2-1_LBC/'
...
#########################################
 ...
 file_clim_in=$path_in_clim'clim-ic_mon-clim_ECMWF-ERA5_rean_r1i1p1_rg_198501-201412.grb'
...
# --------------------
 ...
 for ((yy=fy;yy<=ly;yy++)); do   # year loop
  ...
  # ---------------------------------
   ...
   #201001010600-201007010000
   #201007010600-201101010000

   #2011 2011
   #2011 2012 

    if [ $yy -eq ${fy_file_ta[$nn]} -a $yy -le ${ly_file_ta[$nn]} ]; then
  ...
  # --- ta ----
   ...
   #$cdo -L -invertlev ${file_ta[$((nn))]} $file_ta_yr
   #$cdo -invertlev ${file_ta[$((nn))]} $file_ta_yr
   #rsync ${file_ta[$((nn))]} $file_ta_yr
  ...
  # --- ps ---
   ...
   #ncks -O -v ps ${file_ta[$((nn))]} $file_ps_yr
 ...
 # --- temporary file names ---
  ...
  ########### PROCESSING TIME STEPS ####################
   ...
   # --- one time step for ta ---
    ...
    file_ta_tmp0=$path_out'tmp0_ta_'$yy'_'$ss'.grb'    
    [ -f $file_ta_tmp ] && rm $file_ta_tmp
    #$cdo -L -f grb -setparam,130.128 -setzaxis,$file_zaxis -seltimestep,$ss $file_ta_yr $file_ta_tmp
    
    $cdo -L -seltimestep,$ss $file_ta_yr $file_ta_tmp
    ncpdq -O -a lev,bnds $file_ta_tmp $file_ta_tmp
    $cdo -L -f grb -setparam,130.128 -setzaxis,$file_zaxis $file_ta_tmp $file_ta_tmp0
...
# ########### MERGING ALL VARIABLES to ONE FILE ####################
 ...
 $cdo -remapbil,$grid_out -merge $file_ta_tmp $file_ua_tmp $file_va_tmp $file_hus_tmp $file_ps_tmp $file_orog_tmp $file_sftlf_tmp $file_ts_tmp $file_out_tmp1
