...
# --- Month Loop ---
 ...
 # --- tropk file --- 
   #file_trop_in=''.join([path_trop_in,'tropk_Amon_',gcm_name,'_',gcm_exp,'_',gcm_mem,'_',str(yy),mons[mm],'-',str(yy),mons[mm],'.nc'])
   #print(file_trop_in)
 ...
 # --- get variables ---
  ...
  # --- total AOD (troposphere and stratosphere) --- 
   ...
   aod_tot_tmp=tmp.sum(dim="lev") 
   
   time_in=''.join( [str(yy),'-',mons[mm],'-01'] ) 
   dt = datetime.strptime(time_in,"%Y-%m-%d")
   dt = pd.to_datetime(dt)
   aod_tot_tmp=aod_tot_tmp.assign_coords(time = dt)
   aod_tot_tmp = aod_tot_tmp.expand_dims(dim="time")
...
# ---- output file ----   
 ...
 # --- define variable attributes ---
  ...
  aod_tot.attrs['comments']='troposhere+stratosphere' 
  
 # ---clean and convert to dataset ---
  aod_tot=aod_tot.drop_vars('wl')
  #aod_tot=aod_tot.expand_dims(time=tropk.time)
