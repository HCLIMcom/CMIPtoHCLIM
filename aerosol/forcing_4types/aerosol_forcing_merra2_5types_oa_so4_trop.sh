...
####  od550bc, od550dust, od550oa+od550ss, od550so4 (trop+strat)   ####
####                                                               ####    
#######################################################################
...
# ------ SIMULATIONS ----
# -----------------------
  run_in=(NASA-MERRA2_rean_r1i1p1-vl)

  #od550bc_AERmon_NASA-MERRA2_rean_r1i1p1-vl_198001-202012.nc
...
# ----------------------------
 fy_in=(1991)   # first year
 ly_in=(1996)   # last year
...
# --- read simulation info (simulation id) ---
 ...
 if [ ${#gcm_name} -eq 0 -o ${#gcm_exp} -eq 0 -o ${#gcm_mem} -eq 0 ]; then
  echo ... some DRS elemets are not defined TERMINATED
  exit
 fi

 if [ ${gcm_exp} = 'historical' ]; then 
  gcm_exp_out='hist'
 else
  gcm_exp_out=$gcm_exp
 fi
...
#------------------------------
 ...
 #path_in=$path_ref$gcm_name'/'$gcm_exp'/'$gcm_mem'/input/'$freq'/'
  path_in="/home/rossby/imports/obs/MERRA/MERRA2/orig/mon/derived/aod/"
 ...
 #path_out="/home/rossby/boundary/CMIP6/MIROC6/r1i1p1f1/historical/aero/"
  path_out="/home/rossby/imports/obs/MERRA/MERRA2/orig/mon/derived/cams_format/so4_trop_vl/"
...
# ---first and last year in output file ---
 ...
 file_out=$path_out'aero_'$gcm_name'_'$gcm_mem'_'$gcm_exp_out'-so4trop_'$fy_out$mm'.txt'
 file_tmp0=$path_out'tmp0_aero_'$gcm_name'_'$gcm_mem'_'$gcm_exp_out'-so4trop_'$fy_out$mm'.nc'
...
# --- od550oa + od550so4 ---      
     ...
     file_so4=${file_so4/'r1i1p1-vl_'/'r1i1p1-vl-trop_'}
