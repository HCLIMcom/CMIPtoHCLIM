...
# ----------------------------
 fy_in=(1951)   # first year
 ly_in=(1951)   # last year
...
# --- read simulation info (simulation id) ---
 ...
 if [ ${gcm_exp} = 'historical' ]; then gcm_exp_out='hist'; fi
...
#------------------------------
 ...
 path_rat="/nobackup/rossby27/users/sm_petli/cmip6_gcm_aerosol_ens_mean/"
 ...
 #path_out="/nobackup/rossby26/users/sm_grini/TEMP/20220610_cnrm_aer_oa_so4/historical/"
 #path_out="/nobackup/rossby26/users/sm_grini/TEMP/20220610_cnrm_aer_oa_so4/ssp585/"
 #path_out="/nobackup/rossby26/users/sm_grini/TEMP/20220610_cnrm_aer_oa_so4/historical_neg_val_fix/"
 #path_out="/nobackup/rossby26/users/sm_grini/TEMP/all/"

 #path_out="/home/rossby/boundary/CMIP6/CNRM-ESM2-1/r1i1p1f1/historical/aero/"
 #path_out="/home/rossby/boundary/CMIP6/MIROC6/r1i1p1f1/historical/aero/"
 path_out="/nobackup/rossby27/users/sm_grini/TEMP/20230412_od550aer_scale/CNRM-ESM2-1/r1i1p1f1/historical/aero/"

 [ ! -d "$path_out" ] && echo "... Output path doesn't exist, TERMINATED ..." && exit
...
# ---- all files per simulation and variable ----
 mask_file_in=$path_in'od550aer'_$table_in'_'$gcm_name'_'$gcm_exp'_'$gcm_mem'_'$gcm_grid'*.nc'
...
# ---first and last year in output file ---
 ...
 file_ssrt_tmp0=$path_out'tmp0_ssrt_'$gcm_name'_'$gcm_mem'_'$gcm_exp_out'_'$fy_out$mm'.nc'

 [ -f $file_out ] && rm $file_out
 [ -f $file_tmp0 ] && rm $file_tmp0
 [ -f $file_ssrt_tmp0 ] && rm $file_tmp0
...
# --- od550ss ---
     file_aer=${file_in_all[$nn] }
     echo ... now processing ... $file_aer
     $cdo -L -invertlat $remap,$grid_out -selyear,$fy_out -selmon,$mm $file_aer $file_tmp0
     
     file_ssrt=$path_rat'/'$gcm_exp'/'od550ssrt_AERmon_ENSMEAN_historical_gr90N_195001-201412.nc
     echo ... Sea Salt Ratio ... $file_ssrt
     $cdo -L -invertlat -selyear,$fy_out -selmon,$mm $file_ssrt $file_ssrt_tmp0
     
     ncks -A -v od550ssrt $file_ssrt_tmp0 $file_tmp0
     ncap2 -O -s "od550ss=od550aer*od550ssrt;" $file_tmp0 $file_tmp0
     ncks -O -x -v od550aer,od550ssrt $file_tmp0 $file_tmp0
...
# --- od550oa + od550so4 ---      
     ...
     $cdo -L -invertlat $remap,$grid_out -selyear,$fy_out -selmon,$mm -add $file_oa $file_so4 $file_tmp0
