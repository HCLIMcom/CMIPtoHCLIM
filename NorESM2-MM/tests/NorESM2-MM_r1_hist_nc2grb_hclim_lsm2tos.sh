...
# --------------------
 ...
 for ((yy=fy;yy<=ly;yy++)); do   # year loop
  ...
  # --------------------------
    ...
    file_tos_1st=$path_out'tmp_tos_1st_'$yy'.nc'
    file_lsm=$path_out'tos2lsm_'$yy'.nc'
    [ -f $file_tos_1st ] && rm $file_tos_1st
    [ -f $file_lsm ] && rm $file_lsm
    ncks -d time,0 $file_tos_yr $file_tos_1st
    ncwa -O --no_cll_mth -a time $file_tos_1st $file_tos_1st
    $cdo -L -f nc -ltc,50 -setmisstoc,0. $file_tos_1st $file_lsm
    ncrename -v tos,sftlf -h $file_lsm
    [ -f $file_tos_1st ] && rm $file_tos_1st


   #ncwa -O --no_cll_mth -a time $file_tos_1st $file_tos_1st
   #ncks -A -h -v sftlf $file_sftlf_in $file_tos_1st
   #ncap2 -h -O -s 'sftlf(:, :)=1.;' $file_tos_1st $file_tos_1st 

   #ncap2 -h -O -s 'where(tos=tos.get_miss()) sftlf=0.;' $file_tos_1st $file_tos_1st

   #$cdo -f grb -addc,273.15 -seldate,$aaa_date_in $file_tos $file_tos_tmp0
   #if [ ! -f $file_lsm ]; then
   # $cdo -L -setmisstoc,0. -gtc,50 -addc,100 $file_tos_tmp0 $file_lsm
   #fi
   #$cdo -L -ifthenelse $file_lsm $file_tos_tmp0 $file_ts_tmp $file_tos_tmp1

 
  #ncks -A -h -v sftlf $file_lsm $file_out
  #ncap2 -O -h -s 'where( sftlf < 50 ) '$var_in'=1.e20;' $file_out $file_out
  #ncks -O -h -x -v sftlf $file_out $file_out
 ...
 # --- temporary file names ---
  ...
  ########### PROCESSING TIME STEPS ####################
   ...
   # ---  land sea mask   ---
    ...
    $cdo -L -f grb -setparam,172.128 -settunits,hour -setreftime,$aaa_time_ref -settime,$aaa_time -setdate,$aaa_date $file_lsm $file_sftlf_tmp
