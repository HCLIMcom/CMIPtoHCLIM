...
# --- number of time step (ta file) ---
 ...
 #for ((ss=1;ss<=$((num_ta));ss++)); do   # time step loop
   #for ((ss=1;ss<=4;ss++)); do   # time step loop
 ...
 # --- temporary file names ---
  ...
  ########### PROCESSING TIME STEPS ####################
   ...
   # --- one time step for ta ---
    ...
    $cdo -L -b 16 -f grb2 -setparam,130.128 -setzaxis,$file_zaxis -seltimestep,$ss $file_ta_yr $file_ta_tmp
   ...
   # --- one time step for ua ---
    ...
    $cdo -L -b 16 -f grb2 -setparam,131.128 -setzaxis,$file_zaxis -seldate,$aaa_time_in $file_ua_yr $file_ua_tmp

   # --- one time step for va ---
    file_va_tmp=$path_out'tmp_va_'$yy'_'$ss'.grb' 
    [ -f $file_va_tmp ] && rm $file_va_tmp
    $cdo -L -b 16 -f grb2 -setparam,132.128 -setzaxis,$file_zaxis -seldate,$aaa_time_in $file_va_yr $file_va_tmp 

   # --- one time step for hus ---
    file_hus_tmp=$path_out'tmp_hus_'$yy'_'$ss'.grb' 
    [ -f $file_hus_tmp ] && rm $file_hus_tmp
    $cdo -L -b 16 -f grb2 -setparam,133.128 -setzaxis,$file_zaxis -seldate,$aaa_time_in $file_hus_yr $file_hus_tmp 

   # --- one time step for ps ---
    file_ps_tmp=$path_out'tmp_ps_'$yy'_'$ss'.grb' 
    [ -f $file_ps_tmp ] && rm $file_ps_tmp
    $cdo -L -b 16 -f grb2 -setparam,134.128 -seldate,$aaa_time_in $file_ps_yr $file_ps_tmp

   # --- one time step for lnps ---
    file_lnps_tmp=$path_out'tmp_lnps_'$yy'_'$ss'.grb' 
    [ -f $file_lnps_tmp ] && rm $file_lnps_tmp
    $cdo -L -b 16 -f grb2 -setparam,152.128 -ln -seldate,$aaa_time_in $file_ps_yr $file_lnps_tmp

   # --- one time step for tos ---
    file_tos_tmp=$path_out'tmp_tos_'$yy'_'$ss'.grb' 
    [ -f $file_tos_tmp ] && rm $file_tos_tmp
    $cdo -L -b 16 -f grb2 -setparam,34.128 -seldate,$aaa_time_in $file_tos_yr $file_tos_tmp

   # --- one time step for sic ---
    file_sic_tmp=$path_out'tmp_sic_'$yy'_'$ss'.grb' 
    [ -f $file_sic_tmp ] && rm $file_sic_tmp
    $cdo -L -b 16 -f grb2 -setparam,31.128 -seldate,$aaa_time_in $file_sic_yr $file_sic_tmp
   ...
   # ---    orography   ---
    ...
    $cdo -L -b 16 -f grb2 -mulc,9.80665 -setparam,129.128 -settunits,hour -setreftime,$aaa_time_ref -settime,$aaa_time -setdate,$aaa_date $file_orog_in $file_orog_tmp

   # ---  land sea mask   ---
    file_sftlf_tmp=$path_out'tmp_sftlf_'$yy'_'$ss'.grb' 
    [ -f $file_sftlf_tmp ] && rm $file_sftlf_tmp
    $cdo -L -b 16 -f grb2 -divc,100 -setparam,172.128 -settunits,hour -setreftime,$aaa_time_ref -settime,$aaa_time -setdate,$aaa_date $file_sftlf_in $file_sftlf_tmp
   ...
   # --- IC climatology ERA5  ---
    ...
    [ -f $file_clim_tmp ] && rm $file_clim_tmp
    $cdo -L -b 16 -f grb2 -remapdis,$file_grid -settunits,hour -setreftime,$aaa_time_ref -settime,$aaa_time -setdate,$aaa_date -selmon,$aaa_mon_in $file_clim_in $file_clim_tmp
