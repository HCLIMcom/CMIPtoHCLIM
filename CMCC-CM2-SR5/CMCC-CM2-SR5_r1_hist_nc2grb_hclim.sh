...
#######################################################################
####                                                               ####
####    CMCC-CM2-SR5 output (nc) to HCLIM input files (grb)        ####
####                            CMIP6                              #### 
####                                                               ####  
#######################################################################
...
# ---     EXPERIMENT       ---
#-----------------------------
 grb_name='CMCC-CM2-SR5_r1i1p1f1_hist'
 
 gcm_name='CMCC-CM2-SR5'
 exp_id='historical'
 gcm_mem='r1i1p1f1'
...
#------------------------------
 path_ref_in='/home/rossby/boundary/CMIP6/CMCC-CM2-SR5/r1i1p1f1/historical/nc/'

 #path_out='/nobackup/rossby24/users/sm_grini/Data/CMIP6/LBC/CMCC-CM2-SR5/r1i1p1f1/historical/grb/1971-1985/2001-2014/'
 #path_out='/nobackup/rossby24/users/sm_grini/Data/CMIP6/LBC/CMCC-CM2-SR5/r1i1p1f1/historical/grb/'

 #path_out='/nobackup/rossby24/users/sm_grini/Data/TEMP/20230504_cmcc_lbc/1950/'
 #path_out='/nobackup/rossby24/users/sm_grini/Data/TEMP/20230504_cmcc_lbc/1951-1953/'
 #path_out='/nobackup/rossby24/users/sm_grini/Data/TEMP/20230504_cmcc_lbc/1954-1956/'
 path_out='/nobackup/rossby24/users/sm_grini/Data/TEMP/20230504_cmcc_lbc/1957/'
...
# -----------------
 ...
 file_zaxis=./zaxis/CMCC-CM2-SR5_zaxis_grb_reverse
...
#########################################
 ...
 ### sea points are missing values in orog ###
 ### cdo setmisstoc,0 orog_fx_CMCC-CM2-SR5_historical_r1i1p1f1_gn.nc orog_fx_CMCC-CM2-SR5_historical_r1i1p1f1_gn_fix-miss.nc
 #file_orog_in=$path_in_fx'orog_fx_'$gcm_name'_'$exp_id'_'$gcm_mem'_gn.nc'
 file_orog_in=$path_in_fx'orog_fx_'$gcm_name'_'$exp_id'_'$gcm_mem'_gn_fix-miss.nc'
...
#########################################
   ...
   # --- dates to extract one year ($yy)  ----
    ...
    echo
    echo ... tos first date to extract'    '$f_date_tos 
    echo ... tos last date to extract'     '$l_date_tos
    echo
  ...
  for ((nn=1;nn<=num_file_sic;nn++)); do
   ...
   # --- dates to extract one year ($yy)  ----
    f_date_sic=$((yy-1))'-12-31T00:00:00'
    l_date_sic=$((yy+1))'-01-01T23:00:00'
    echo
    echo ... sic first date to extract'    '$f_date_sic 
    echo ... sic last date to extract'     '$l_date_sic
    echo
 ...
 # --- dates to interpolate daily to 6hr ----
  ...
  file_ta=$path_in_atmos'ta_6hrLev_'$gcm_name'_'$exp_id'_'$gcm_mem'_gn_'$yy'01010600-'$((yy+1))'01010000.nc' 
...
# --- number of time step (ta file) ---
 ...
 for ((ss=1;ss<=$((num_ta));ss++)); do   # time step loop
   #for ((ss=1;ss<=1;ss++)); do   # time step loop
    #for ((ss=836;ss<=972;ss++)); do   # time step loop
