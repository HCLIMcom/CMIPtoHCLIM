...
#------------------------------
 ...
 path_out='/nobackup/rossby24/users/sm_grini/Data/CMIP6/LBC/MPI-ESM1-2-LR/grb/1958-1958/'
...
exit
  ...
  # --- ts ---
  # ----------
    file_ts_tmp0=$path_out'tmp0_ts_'$grb_name'_'$yy'.nc'
    file_ts_yr=$path_out'ts_'$grb_name'_'$yy'.nc'
    [ -f $file_ts_tmp0 ] && rm $file_ts_tmp0
    [ -f $file_ts_yr ] && rm $file_ts_yr

    if [ $yy -eq ${fy_file_tos[$nn]} ]; then
     echo ... FIRST YEAR ts ...
     $cdo -L -seldate,$f_date_tos,$l_date_tos ${file_ts[$((nn-1))]} $file_ts_tmp0
    fi
  
    $cdo -L cat -seldate,$f_date_tos,$l_date_tos ${file_ts[$((nn))]} $file_ts_tmp0

    if [ $yy -eq ${ly_file_tos[$nn]} ]; then
     echo ... LAST YEAR ts ...
     $cdo -L -cat -seldate,$f_date_tos,$l_date_tos ${file_ts[$((nn+1))]} $file_ts_tmp0
    fi
   ...
   et="$(date +%s)"
   echo
   echo ... 'Extracting one year of Ocean + ts DONE, time : ... ' "$(expr $et - $bt)" sec 
   fi
